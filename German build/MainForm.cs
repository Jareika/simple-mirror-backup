...
public sealed class MainForm : Form
{
    ...
    private void InitializeUi()
    {
        ...
        txtName.PlaceholderText = "Jobname";

        txtSource.Dock = DockStyle.Fill;
        txtSource.PlaceholderText = @"C:\Quelle oder \\Server\Freigabe\Quelle";

        txtTarget.Dock = DockStyle.Fill;
        txtTarget.PlaceholderText = @"D:\Ziel oder \\Server\Freigabe\Ziel";
        ...
        btnRefreshTree.Text = "Ordner laden";
        btnSwapDirection.Text = "Richtung tauschen";

        btnNew.Text = "Neu";
        btnCopy.Text = "Kopieren";
        btnCopyReverse.Text = "Kopie ↔";
        btnRename.Text = "Umbenennen";
        btnDelete.Text = "Löschen";
        btnCompare.Text = "Vergleichen ▼";
        btnRun.Text = "Spiegeln";
        btnSync.Text = "Synchronisieren";
        btnBackup.Text = "Backup";
        btnNewFolder.Text = "Neuer Ordner";
		btnNewRootFolder.Text = "Neuer Hauptordner";
        ...
        var lblSource = new Label { Text = "Quelle", AutoSize = true, Anchor = AnchorStyles.Left };
        var lblTarget = new Label { Text = "Ziel", AutoSize = true, Anchor = AnchorStyles.Left };
        var lblTree = new Label
        {
            Text = "Unterordner: abgewählte Ordner werden im Lauf nicht gescannt",
            AutoSize = true,
            Anchor = AnchorStyles.Left
        };
        var lblLog = new Label { Text = "Protokoll", AutoSize = true, Anchor = AnchorStyles.Left };
    ...
    private void WireEvents()
    {
            ...
            if (!PathsEquivalent(CurrentJob.SourcePath, newSource))
            {
                ...
                ResetFolderTreeState(
                    string.IsNullOrWhiteSpace(newSource)
                        ? "Bitte Quellordner wählen."
                        : "Quellordner geändert. Bitte 'Ordner laden' klicken.");
            }
        ...
        compareMenu.Items.Add("Spiegeln vergleichen", null, async (_, _) => await CompareCurrentJobAsync(BackupMode.Mirror));
        compareMenu.Items.Add("Synchronisieren vergleichen", null, async (_, _) => await CompareCurrentJobAsync(BackupMode.Synchronize));
        compareMenu.Items.Add("Backup vergleichen", null, async (_, _) => await CompareCurrentJobAsync(BackupMode.Backup));
    ...
    private void CreateFolderUnder(string parentPath)
    {
        ...
        var initialName = GetUniqueFolderName(parentPath, "Neuer Ordner");

        var newName = PromptDialog.Show(
            this,
            string.IsNullOrWhiteSpace(parentPath) ? "Hauptordner anlegen" : "Ordner anlegen",
            "Ordnername:",
            initialName);
    ...
    private void RenameSelectedFolder(string folderPath)
    {
        ...
        var newName = PromptDialog.Show(this, "Ordner umbenennen", "Name:", currentName);
        if (newName is null)
            return;
    ...
    private void TreeJobs_DragDrop(object? sender, DragEventArgs e)
    {
        ...
        lblStatus.Text = string.IsNullOrWhiteSpace(targetFolderPath)
            ? "Job in die Hauptebene verschoben."
    ...
    private async Task LoadFolderTreeAsync()
    {
        ...
        if (job is null)
        {
            ResetFolderTreeState("Kein Job ausgewählt.");
            return;
        }
        ...
        {
            ResetFolderTreeState("Bitte Quellordner wählen.");
            return;
        }
        ...
        lblStatus.Text = "Ordner werden geladen...";
        ...
        try
        {
            ...
            lblStatus.Text = "Ordner geladen.";
        }
        catch (DirectoryNotFoundException)
        {
            ResetFolderTreeState("Quellordner nicht gefunden.");
        }
        catch (OperationCanceledException)
        {
            if (CurrentJob is not null)
                lblStatus.Text = "Ordnerladen abgebrochen.";
        }
        catch (Exception ex)
        {
            ResetFolderTreeState("Ordnerbaum konnte nicht geladen werden.");
            AppendLog("Ordnerbaum-Fehler: " + ex.Message);
        }
        finally
        {
    ...
    private void PopulateEditorFromCurrentJob()
    {
        ...
        try
        {
            ...
            if (!hasJob)
            {
                ...
                if (selectedNode?.Tag is string folderPath)
                    lblStatus.Text = $"Ordner ausgewählt: {folderPath}";
                else
                    lblStatus.Text = "Kein Job ausgewählt.";
        ...
        ResetFolderTreeState(
            string.IsNullOrWhiteSpace(job.SourcePath)
                ? "Bitte Quellordner wählen."
                : "Ordnerbaum für diesen Job nicht geladen. Bitte 'Ordner laden' klicken.");
    }
    ...
    private void CopyCurrentJob(bool reverseDirection)
    {
        ...
        clone.Name = reverseDirection
            ? GetUniqueJobName(current.Name + " (Kopie rückwärts)")
            : GetUniqueJobName(current.Name + " (Kopie)");
    ...
    private void RenameCurrentJob()
    {
        ...
        var newName = PromptDialog.Show(this, "Job umbenennen", "Name:", current.Name);
        if (newName is null)
            return;
    ...
    private void DeleteCurrentJob()
    {
        ...
        var result = MessageBox.Show(
            this,
            $"Job '{current.Name}' wirklich löschen?",
            "Job löschen",
            MessageBoxButtons.YesNo,
    ...
    private void SwapDirection()
    {
        ...
        CancelFolderLoad();
        ResetFolderTreeState("Richtung getauscht. Bitte 'Ordner laden' klicken.");
        SaveAllJobs();
    }
        ...
        CancellationToken cancellationToken)
    {
        ...
        {
            AppendLog($"Ordnerbaum: '{fullPath}' konnte nicht gelesen werden: {ex.Message}");
            return;
        }
    ...
    private async Task CompareCurrentJobAsync(BackupMode mode)
    {
        ...
        lblStatus.Text = "Vergleich läuft...";

        try
        {
            var plan = await BuildPlanAsync(current, mode);
            lblStatus.Text = $"{GetModeDisplayName(mode)}: {plan.CopyCount} Kopieren, {plan.DeleteCount} Löschen.";
        ...
        catch (Exception ex)
        {
            ...
            lblStatus.Text = "Vergleich fehlgeschlagen.";
    ...
    private async Task RunCurrentJobAsync(BackupMode mode)
    {
        ...
        AppendLog($"Modus: {GetModeDisplayName(mode)}");

        ToggleEditorEnabled(false);
        lblStatus.Text = $"{GetModeDisplayName(mode)} läuft...";
        ...
        try
        {
            ...
            lblStatus.Text = $"{GetModeDisplayName(mode)} fertig.";
        }
        catch (Exception ex)
        {
            AppendLog("FEHLER: " + ex.Message);
    ...
    private static string GetModeDisplayName(BackupMode mode)
    {
        return mode switch
        {
            BackupMode.Mirror => "Spiegeln",
            BackupMode.Synchronize => "Synchronisieren",
    ...
    private void SaveAllJobs(bool showStatus = true)
    {
        try
        {
            ...
            if (showStatus)
                lblStatus.Text = $"Gespeichert {DateTime.Now:HH:mm:ss}";
        }
        catch (Exception ex)
        {
            AppendLog("Speicherfehler: " + ex.Message);

            if (showStatus)
                lblStatus.Text = "Speichern fehlgeschlagen.";
        }
    }
    ...
    private void UpdateCurrentJobFromEditor()
    {
        ...
        CurrentJob.Name = string.IsNullOrWhiteSpace(txtName.Text) ? "Unbenannter Job" : txtName.Text.Trim();
    ...
    {
        var name = string.IsNullOrWhiteSpace(baseName) ? "Ordner" : baseName.Trim();
