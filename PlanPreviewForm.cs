...
public sealed class PlanPreviewForm : Form
{
    ...
    public PlanPreviewForm(BackupPlan plan)
    {
        ...
        Text = $"Comparison - {GetModeText(plan.Mode)}";
        Width = 1100;
        Height = 720;
    ...
    private static string BuildSummary(BackupPlan plan)
    {
        ...
        var baseText = plan.TotalCount == 0
            ? $"{modeText}: no changes required."
            : $"{modeText}: {plan.CopyCount} copy actions, {plan.DeleteCount} delete actions, {plan.TotalCount} changes total.";

        return plan.Warnings.Count == 0
            ? baseText
            : baseText + $" Warnings: {plan.Warnings.Count}.";
    }
    ...
    private static string GetModeText(BackupMode mode)
    {
        return mode switch
        {
            BackupMode.Mirror => "Mirror comparison",
            BackupMode.Synchronize => "Synchronization comparison",
            BackupMode.Backup => "Backup comparison",
            _ => "Comparison"
        };
    }
}